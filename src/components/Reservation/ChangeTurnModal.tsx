...
import { reservationTimes, updateReservation } from "../../service/api/Reserve";
...
const ChangeTurnModal = (props: any) => {
  //! Interfaces
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      getReservationTimesRefetch();
    }, 2);
  }, [props.data]);

  //! Requests
  const { mutate: updateReserveMutate } = useMutate(updateReservation);

  console.log("-+-", props.data?.details?.[0]);

  const {
    data: reservationTimesData,
    isLoading,
    refetch: getReservationTimesRefetch,
  } = useGet(
    reservationTimes,
    ["reservationTimes"],
  ...
  const updateReserveHandler = () => {
    updateReserveMutate(
      ...
      {
        onSuccess() {
          addToast(" جابجایی نوبت با موفقیت انجام گردید ", "success");
        },
        onError() {
          addToast(" جابجایی نوبت با مشکل روبرو شد", "error");
        },
      }
    );
    setTimeout(() => {
    ...
    closeModalHandler("changeTurnModal");
  };
  return (
    <Modal
      id={"changeTurnModal"}
      title="جابجایی نوبت"
      clickHandler={updateReserveHandler}
      textPrimaryBtn="جابجایی نوبت"
      wrapperClassName="!max-w-[58.5rem] !overflow-hidden"
    >
          ...
          <section className="flex flex-col sm:flex-row gap-4 w-full h-full max-w-[58.125rem] mx-auto sm:h-[32.5rem] sm:px-5">
            ...
            <div className="flex flex-col gap-y-2 sm:gap-y-4 w-full h-full px-5 sm:px-0">
              <p className="text-sm font-semibold text-danger-500 rounded-xl py-[0.625rem] bg-danger-50 text-center">
                10 دقیقه مهلت دارید تا مراحل رزرو را تکمیل کنید در غیر این صورت
